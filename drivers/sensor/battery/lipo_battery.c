...
// with the only change being the percentage lookup table changed to that of lipo batteries (Ln 82),
// the splitkb_battery_common.h included for the lookup function (Ln 20). Longer int time (Ln 67) and ADC_GAIN (Ln 148)

#define DT_DRV_COMPAT splitkb_lipo_voltage_divider
...
static int bvd_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    if (rc == 0) {
        ...
        uint8_t percent = skb_lipo_mv_to_pct(millivolts);
